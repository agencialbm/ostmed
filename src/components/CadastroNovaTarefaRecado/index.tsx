...
export function CadastroNovaTarefaRecado({
  visible,
  onClose
}: ITarefaRecadoModal) {
  const { tarefaVisibleCard } = useContext(TarefaCardContext);
  ...
  const { closeModalTarefaRecado } = useContext(TarefaRecadoModalContext);
      ...
      <Overlay>
        <Container>
          <button
            ...
            onClick={() => closeModalTarefaRecado()}
          >
            <Icon
              name="closeSelect"
              size={13}
