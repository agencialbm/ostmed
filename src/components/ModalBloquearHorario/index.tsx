...
import { CustomInputCadastro } from '../CustomInputCadastro';
import { Toggle } from '../Toggler';
import { ToggleOn } from '../TogglerOn';
import { DayOfWeekRadio } from '../RaioDias';
...
export function ModalBloquearHorario({ visible, onClose }: IOrderModal) {
  const [selectedValue, setSelectedValue] = useState('');
  // const { closeModalBloquearHorario } = useContext(ModalCadastroContext);
  const { closeModalBloquearHorario } = useContext(BloquearHorarioContext);
  const [inputValue, setInputValue] = useState('');
  const [isOn2, setIsOn2] = useState<boolean>(false);
  const [isOff, setIsisOff] = useState<boolean>(true);
  ...
  const handleToggleOff = (newIsOn: boolean) => {
    setIsisOff(newIsOn);
  };
        ...
        <ContainerModal>
          <div className="div1Modal">
            ...
            <section className="calendario">
              <div>
                <p>AGENDAR</p>
                <Toggle isOn={isOff} onToggle={handleToggleOff} />
                <text>BLOQUEAR</text>
              </div>
          ...
          {pesquisa ? (
            <div className="div2Modal">
              ...
              <aside>
                ...
                <button className="confirm">Confirmar bloqueio</button>
              </aside>
            </div>
          ) : (
            <ModalRegistro>
              ...
              <ModalValor>
                <p>
                  Uma receita será lançada automaticamente no financeiro quando
                  o agendamento for concluido.
                </p>
              ...
              <ModalBtn>
                <button onClick={closePesquisa}>Voltar</button>
                <div>
